...
function optional_field(key3, default$, field_decoder, next) {
  return new Decoder(
    (data) => {
      let _block;
      let _block$1;
...
function from_list_loop(loop$list, loop$initial) {
  while (true) {
    ...
    if (list4 instanceof Empty) {
      ...
      loop$list = rest;
      loop$initial = insert(initial, key3, value3);
    }
  }
}
  ...
  while (true) {
    ...
    if (uri_string.startsWith("#")) {
      if (size2 === 0) {
        ...
        let rest = uri_string.slice(1);
        let query2 = string_codeunit_slice(original, 0, size2);
...
function merge2(base, relative2) {
  let $ = base.host;
  if ($ instanceof Some) {
    let $1 = base.scheme;
    if ($1 instanceof Some) {
      ...
      if ($2 instanceof Some) {
        let _block;
        let _pipe = split2(relative2.path, "/");
        ...
        if ($4 === "") {
          ...
          if ($5) {
            _block$1 = split2(relative2.path, "/");
          } else {
            let _pipe2 = split2(base.path, "/");
            let _pipe$12 = drop_last(_pipe2);
            _block$1 = append(_pipe$12, split2(relative2.path, "/"));
          }
...
function merge3(loop$attributes, loop$merged) {
  while (true) {
...
function prepare(attributes) {
  if (attributes instanceof Empty) {
    ...
    if ($ instanceof Empty) {
      ...
      return merge3(_pipe$1, empty_list);
    }
  }
}
...
var Runtime = class {
  constructor(root3, [model, effects], view11, update9) {
    this.root = root3;
    this.#model = model;
    this.#view = view11;
    this.#update = update9;
...
var make_component = ({ init: init9, update: update9, view: view11, config }, name2) => {
...
var Spa = class {
  #runtime;
  constructor(root3, [init9, effects], update9, view11) {
    this.#runtime = new Runtime(root3, [init9, effects], view11, update9);
  }
  send(message2) {
    switch (message2.constructor) {
      case EffectDispatchedMessage: {
...
var start = ({ init: init9, update: update9, view: view11 }, selector, flags) => {
  if (!is_browser()) return new Error2(new NotABrowser());
  const root3 = selector instanceof HTMLElement ? selector : document2().querySelector(selector);
  if (!root3) return new Error2(new ElementNotFound(selector));
  return new Ok(new Spa(root3, init9(flags), update9, view11));
};

// build/dev/javascript/lustre/lustre.mjs
var App = class extends CustomType {
  constructor(init9, update9, view11, config) {
    super();
    this.init = init9;
    this.update = update9;
    this.view = view11;
...
function component(init9, update9, view11, options) {
  return new App(init9, update9, view11, new$7(options));
}
function application(init9, update9, view11) {
  return new App(init9, update9, view11, new$7(empty_list));
}
...
var FILEPATH = "src/somachord/storage.gleam";
...
var FILEPATH2 = "src/somachord/router.gleam";
  ...
  if ($ === "/") {
    ...
    if ($1 instanceof Ok) {
      ...
      throw makeError(
        ...
        "uri_to_route",
        "Pattern match failed, no pattern matched the value.",
        { value: $1, start: 459, end: 515, pattern_start: 470, pattern_end: 487 }
      );
    }
...
var Model = class extends CustomType {
  constructor(route, layout, storage, confirmed, albums, player, queue2, current_song, seeking, seek_amount, played_seconds) {
    super();
    this.route = route;
    this.layout = layout;
          ...
          return optional_field(
            ...
            (cover_art_id) => {
              return optional_field(
                ...
                (albums) => {
                  return success(
                    new Artist2(id2, name2, cover_art_id, albums)
                  );
                }
              );
            }
...
var FILEPATH3 = "src/somachord/api_helper.gleam";
var Ping = class extends CustomType {
};
var Artist3 = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var TopSongs = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
var Album3 = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var Scrobble = class extends CustomType {
};
...
var SubsonicError = class extends CustomType {
  constructor(code2, message2, attempted_route) {
    ...
    this.code = code2;
    this.message = message2;
    this.attempted_route = attempted_route;
  }
};
function create_uri(path, auth_details, query2) {
  ...
  if ($ instanceof Ok) {
    ...
    throw makeError(
      ...
      "create_uri",
      "Pattern match failed, no pattern matched the value.",
      { value: $, start: 694, end: 752, pattern_start: 705, pattern_end: 713 }
    );
  }
  ...
  if ($1 instanceof Ok) {
    ...
    throw makeError(
      ...
      "create_uri",
      "Pattern match failed, no pattern matched the value.",
      { value: $1, start: 755, end: 821, pattern_start: 766, pattern_end: 782 }
    );
  }
  return new Uri(
    original_uri.scheme,
...
function construct_req(auth_details, path, query2, decoder2, msg) {
  let $ = to(
    (() => {
      let _pipe = create_uri(path, auth_details, query2);
            ...
            return subfield(
              ...
              (code2) => {
                return subfield(
                  ...
                  (message2) => {
                    return success(
                      new SubsonicError(code2, message2, path)
                    );
                  }
                );
              }
...
// build/dev/javascript/somachord/somachord/api.mjs
var FILEPATH4 = "src/somachord/api.gleam";
...
function album_list(auth_details, type_2, offset, amount) {
  return construct_req(
    auth_details,
...
function album(auth_details, id2) {
  return construct_req(
    ...
    subfield(
      ...
      album_decoder(),
      (album3) => {
        return success(new Album3(album3));
      }
    ),
    (var0) => {
      return new SubsonicResponse(var0);
...
function artist(auth_details, id2) {
  return construct_req(
    ...
    subfield(
      ...
      artist_decoder(),
      (artist2) => {
        return success(new Artist3(artist2));
      }
    ),
    (var0) => {
      return new SubsonicResponse(var0);
...
function song(auth_details, id2, msg) {
  return construct_req(
    ...
    subfield(
      ...
      song_decoder(),
      (song3) => {
        return success(new Song2(song3));
      }
    ),
    msg
  );
}
function top_songs(auth_details, artist_name) {
  return construct_req(
    auth_details,
    ...
    subfield(
      ...
      list2(song_decoder()),
      (songs) => {
        return success(new TopSongs(songs));
      }
    ),
    (var0) => {
      return new SubsonicResponse(var0);
...
function search(auth_details, query2) {
  return construct_req(
    auth_details,
    "/rest/search3.view",
    toList([["query", query2]]),
        ...
        return then$(
          ...
          (albums) => {
            return success(
              new Search2(
                artists,
                filter(albums, (album3) => {
                  return album3.year !== 0;
                }),
                toList([])
              )
            );
...
function similar_songs(auth_details, id2) {
  return construct_req(
    ...
    subfield(
      ...
      list2(song_decoder()),
      (songs) => {
        return success(new SimilarSongs(songs));
      }
    ),
    (var0) => {
      return new SubsonicResponse(var0);
...
var FILEPATH5 = "src/somachord/components/login.gleam";
...
var SomachordMsg = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function update2(m, msg) {
  if (msg instanceof LoginSubmitted) {
    ...
    if ($ instanceof Ok) {
      ...
      return [
        new Model2(m.storage, m.login_form, auth),
        (() => {
          let _pipe = ping(auth);
          return map4(_pipe, (a2) => {
            return new SomachordMsg(a2);
          });
        })()
      ];
    } else {
      let updated_form = $[0];
      return [
        new Model2(m.storage, updated_form, m.auth_details),
        none2()
      ];
    }
  } else {
...
var FILEPATH6 = "src/somachord/elements.gleam";
  ...
  let _block;
  {
    ...
    if ($ instanceof Ok) {
      ...
      throw makeError(
        ...
        "song",
        "Pattern match failed, no pattern matched the value.",
        { value: $, start: 468, end: 528, pattern_start: 479, pattern_end: 486 }
      );
    }
    _block = stg.auth;
  }
...
var FILEPATH7 = "src/somachord/pages/album.gleam";
        ...
        toList([
          div(
            ...
            toList([
              ...
              div(
                toList([
                  class$("text-zinc-400 flex gap-4 items-center")
                ]),
                toList([
                  i(
                    toList([
                      ...
                      on_click(new StreamAlbum(album3))
                    ]),
                    toList([])
                  ),
                  i(
...
var FILEPATH8 = "src/somachord/pages/artist.gleam";
...
var SomachordMsg2 = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function update4(m, msg) {
  if (msg instanceof ChangeTab2) {
    let tab = msg[0];
    return [
      new Model4(tab, m.artist, m.artist_id, m.top_songs, m.auth_details),
      none2()
    ];
  ...
  } else if (msg instanceof SomachordMsg2) {
    let $ = msg[0];
    if ($ instanceof SubsonicResponse) {
      let $1 = $[0];
      if ($1 instanceof Ok) {
        let $2 = $1[0];
        if ($2 instanceof Artist3) {
          let artist2 = $2[0];
          let $3 = m.auth_details;
          let auth_details;
          ...
          return [
            ...
            (() => {
              let _pipe = top_songs(auth_details, artist2.name);
              return map4(
                _pipe,
                (var0) => {
                  return new SomachordMsg2(var0);
        ...
        let e = $1[0];
        echo(e, void 0, "src/somachord/pages/artist.gleam", 119);
        return [m, none2()];
      }
    } else {
      return [m, none2()];
          ...
          div(
            ...
            toList([
              ...
              div(
                toList([
                  class$(
                    "z-20 flex items-center justify-between w-full"
                  )
                ]),
                toList([
                  div(
                    ...
                    toList([
                      h1(
                        toList([class$("font-extrabold text-5xl")]),
                        toList([
                          text2(
                            (() => {
                              let $ = (() => {
...
function update5(m, msg) {
  if (msg instanceof SubsonicResponse) {
    let $ = msg[0];
    if ($ instanceof Ok) {
      let $1 = $[0];
      if ($1 instanceof AlbumList) {
        let type_2 = $1[0];
        let list4 = $1[1];
        return [
          new Model5(
            (() => {
              let _pipe = prepend(new AlbumList2(type_2, list4), m.albumlists);
              return sort(
                _pipe,
                (list1, list22) => {
      ...
      let e = $[0];
      echo2(e, void 0, "src/somachord/pages/home.gleam", 85);
      return [m, none2()];
    }
...
function view5(m) {
  return div(
    toList([
      redirect_click(new ComponentClick()),
...
var FILEPATH9 = "src/somachord/pages/search.gleam";
...
function init6(_) {
  return [new Model6("", toList([]), toList([])), none2()];
}
function update6(m, msg) {
  ...
  if (msg instanceof Search4) {
    ...
    return [
      m,
      (() => {
        let _pipe = search(auth_details, query$1);
        return map4(
          _pipe,
          (msg2) => {
            echo3(query$1, void 0, "src/somachord/pages/search.gleam", 76);
            if (msg2 instanceof SubsonicResponse) {
              let $ = msg2[0];
              if ($ instanceof Ok) {
                let $1 = $[0];
                if ($1 instanceof Search2) {
                  let albums = $1.albums;
                  return new SearchResults(albums);
                } else {
                  echo3(msg2, void 0, "src/somachord/pages/search.gleam", 81);
                  return new Nothing3();
                }
              } else {
                echo3(msg2, void 0, "src/somachord/pages/search.gleam", 81);
...
var FILEPATH10 = "src/somachord/pages/song.gleam";
  ...
  if (msg instanceof SongID) {
    ...
    if ($ instanceof Ok) {
      ...
      if ($1 instanceof Song2) {
        ...
        let code2 = $1.code;
        let msg$1 = $1.message;
        echo4(msg$1, void 0, "src/somachord/pages/song.gleam", 65);
        echo4(code2, void 0, "src/somachord/pages/song.gleam", 66);
        return [m, none2()];
      } else {
        return [m, none2()];
      }
...
function view7(song3) {
  ...
  let auth_details = _block;
  return div(
    toList([redirect_click(new Nothing())]),
    toList([
      div(
...
var FILEPATH11 = "src/somachord/view/desktop.gleam";
function top_bar(m) {
  return div(
      ...
      div(
        ...
        toList([
          div(
            ...
            toList([
              i(
                toList([class$("text-xl ph ph-shuffle-simple")]),
                toList([])
              ),
              i(
                toList([
          ...
          div(
            ...
            toList([
              ...
              div(
                toList([class$("grid grid-cols-1 grid-rows-1 w-96")]),
                toList([
                  div(
                    toList([
                      class$(
                    ...
                    map(
                      ...
                      (queue_entry) => {
                        return song2(
                          ...
                          toList([]),
                          true,
                          new StreamFromQueue(queue_entry[0])
                        );
                      }
                    )
                  )
...
// build/dev/javascript/somachord/somachord/view/mobile.mjs
        ...
        toList([
          div(
            ...
            toList([
              ...
              mobile_nav_button(
                i(
                  toList([class$("text-3xl ph ph-sparkles")]),
                  toList([])
                ),
                i(
                  toList([class$("text-3xl ph-fill ph-sparkles")]),
                  toList([])
                ),
                "Discover",
                false,
...
var FILEPATH12 = "src/somachord.gleam";
function route_effect(m, route) {
  if (route instanceof Album) {
    let id2 = route.id;
    let _block;
    {
      let _block$1;
...
function play_from_queue(position) {
  return from(
    (dispatch) => {
      let _pipe = new StreamFromQueue(position);
      return dispatch(_pipe);
    }
  );
}
function update8(m, msg) {
  if (msg instanceof Router) {
  ...
  } else if (msg instanceof SubsonicResponse) {
    let $ = msg[0];
    if ($ instanceof Ok) {
      let $1 = $[0];
      if ($1 instanceof Album3) {
        let album3 = $1[0];
        return [
          new Model(
            m.route,
      ...
      } else if ($1 instanceof SimilarSongs) {
        let songs = $1[0];
        let _block;
        {
          let _block$12;
        ...
        let auth_details = _block;
        echo6(m.queue.position + 1, void 0, "src/somachord.gleam", 353);
        let _block$1;
        let _pipe = songs;
        let _pipe$1 = filter(
          _pipe,
          (song3) => {
            let _pipe$12 = m.queue.songs;
            let _pipe$22 = values(_pipe$12);
            let _pipe$32 = find2(
              _pipe$22,
              (potential_song) => {
                return potential_song.id === song3.id;
              }
            );
        ...
        let _pipe$2 = fold(
          ...
          [
            new_map(),
            (() => {
              let _pipe$22 = m.queue.songs;
              let _pipe$32 = keys(_pipe$22);
              return length(_pipe$32);
            })()
          ],
          (acc, song3) => {
            let d;
        ...
        let _pipe$3 = first2(_pipe$2);
        _block$1 = merge(_pipe$3, m.queue.songs);
        let new_queue = _block$1;
        return [
          new Model(
            m.route,
            m.layout,
          ...
          (() => {
            let $2 = m.queue.position + 1 === (() => {
              let _pipe$4 = new_queue;
              let _pipe$5 = keys(_pipe$4);
              return length(_pipe$5);
            })();
            if ($2) {
              let _block$2;
              ...
              return similar_songs_artist(auth_details, first_artist.id);
            } else {
              return play_from_queue(m.queue.position + 1);
            }
          })()
        ];
        ...
        echo6(queue2.changed, void 0, "src/somachord.gleam", 149);
        return [
          new Model(
            m.route,
            m.layout,
        ...
        let attempted_path = $1.attempted_route;
        let _block;
        {
          let _block$1;
          let _pipe = m.storage;
        ...
        let auth_details = _block;
        echo6(attempted_path, void 0, "src/somachord.gleam", 391);
        echo6(code2, void 0, "src/somachord.gleam", 392);
        echo6(message2, void 0, "src/somachord.gleam", 393);
        if (attempted_path === "/rest/getSimilarSongs.rest") {
          ...
          return [m, similar_songs_artist(auth_details, first_artist.id)];
        } else {
          return [m, none2()];
        }
      } else {
      ...
      let e = $[0];
      echo6(e, void 0, "src/somachord.gleam", 182);
      return [m, none2()];
    }
    ...
    if ($ === "album") {
      return guard(
        ...
        [
          ...
          from(
            (dispatch) => {
              echo6("already have album", void 0, "src/somachord.gleam", 197);
              let _block$1;
              let _pipe = m.albums;
              ...
              let _pipe$1 = new StreamAlbum(album3);
              return dispatch(_pipe$1);
            }
          )
        ],
        () => {
          return [
            m,
            (() => {
              let _pipe = album(auth_details, req.id);
              return map4(
                _pipe,
                (message2) => {
                  if (message2 instanceof SubsonicResponse) {
                    let $1 = message2[0];
                    if ($1 instanceof Ok) {
                      let $2 = $1[0];
                      if ($2 instanceof Album3) {
                        let album3 = $2[0];
                        return new StreamAlbum(album3);
                      } else {
                        return message2;
                      }
                    } else {
    ...
    let auth_details = _block;
    echo6("getting first song", void 0, "src/somachord.gleam", 228);
    let _block$1;
    let _pipe = album3.songs;
    _block$1 = first(_pipe);
    let $ = _block$1;
            ...
            let _pipe$4 = fold(
              ...
              (acc, song4) => {
                ...
                return [
                  (() => {
                    let _pipe$42 = d;
                    return insert(_pipe$42, idx, song4);
                  })(),
                  idx + 1
                ];
              }
    ...
    let _block$1;
    let _pipe = m.queue.songs;
    _block$1 = map_get(_pipe, position);
    let $ = _block$1;
    let song3;
    if ($ instanceof Ok) {
      song3 = $[0];
    ...
    let time3 = msg.time;
    let _block;
    let $ = absolute_value(time3 - identity(m.played_seconds)) > 1;
    if ($) {
    ...
    let playtime = _block;
    echo6(playtime, void 0, "src/somachord.gleam", 287);
    return [
      new Model(
        m.route,
        m.layout,
    ...
    return [
      ...
      batch(
        toList([
          (() => {
            let $ = m.played_seconds > globalThis.Math.trunc(
              m.current_song.duration / 2
            );
            if ($) {
              return scrobble(
                (() => {
          ...
          (() => {
            ...
            if ($ instanceof Lt) {
              ...
              let auth_details = _block;
              return similar_songs(auth_details, m.current_song.id);
            } else {
              return none2();
            }
          })()
    ...
    return [
      ...
      (() => {
        let $ = m.current_song.starred;
        if ($) {
          return unlike(auth_details, m.current_song.id);
        } else {
          return like(auth_details, m.current_song.id);
        }
      })()
    ];
...
function init8(_) {
  ...
  let layout = _block$1;
  echo6(layout, void 0, "src/somachord.gleam", 65);
  let m = new Model(
    route,
    layout,
    create(),
...
function main() {
  let app = application(init8, update8, view10);
  let $ = register();
  if (!($ instanceof Ok)) {
    throw makeError(
      "let_assert",
      ...
      "main",
      "Pattern match failed, no pattern matched the value.",
      { value: $, start: 891, end: 926, pattern_start: 902, pattern_end: 907 }
    );
  }
  let $1 = register4();
  if (!($1 instanceof Ok)) {
    throw makeError(
      ...
      "main",
      "Pattern match failed, no pattern matched the value.",
      { value: $1, start: 929, end: 963, pattern_start: 940, pattern_end: 945 }
    );
  }
  let $2 = register3();
  if (!($2 instanceof Ok)) {
    throw makeError(
      ...
      "main",
      "Pattern match failed, no pattern matched the value.",
      { value: $2, start: 966, end: 1002, pattern_start: 977, pattern_end: 982 }
    );
  }
  let $3 = register6();
  if (!($3 instanceof Ok)) {
